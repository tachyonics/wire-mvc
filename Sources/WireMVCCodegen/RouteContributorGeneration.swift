...
/// keyless global-middleware proxy's `wrapGlobalMiddleware` extension (M5.5 Phase 5) and the `@main` entry.
/// The extension is rendered here so it sees the full `factoryKeys` set (a global `@Middleware(key)` may
/// reference a factory declared in any file); the `@main` calls it on `graph._WireGlobalMiddleware_<Bootstrap>`.
