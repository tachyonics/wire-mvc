...
/// `WireMVCTesting` product) gets the free `withTestServer` entry (and `import WireMVCTesting`) *instead*
/// — the two are mutually exclusive because a `@main` in a test bundle collides with the runner's own
/// entry. `extraImports` are `import <Module>` lines for the Wire-aware dependency modules whose sources
...
public func generateRouteContributors(
    ...
    // The generated `@main`/`withTestServer` entry calls `Wire.bootstrap()`, so the consumer module needs
    // `import Wire` even when no controller source imported it directly. The `withTestServer` entry (test
    // consumer only) references `WireMVCTesting`, so that import is added only when it is emitted — a
    // program consumer must not link the test client.
...
/// sources (its generated `@main`/`withTestServer` entry) come last, at module scope. Split out of
...
/// one program entry — the `@main` for a program consumer, or the free `withTestServer` entry for a test
/// consumer (`testEntry`). The two entries are mutually exclusive: a `@main` in a test bundle collides with
/// the test runner's own entry point, so a re-composing test target emits `withTestServer` in its place.
...
private func bootstrapArtifacts(
    ...
    // A test consumer gets the free `withTestServer` entry (it inlines the same build and serves via
    // `WireMVCTesting.withTestServer` so a test drives the app over real HTTP); a program consumer gets the
    // `@main`. Never both — the `@main` would collide with the test runner's entry.
    let entry =
        testEntry
