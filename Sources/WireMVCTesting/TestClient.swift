...
// A minimal typed HTTP client over `URLSession`, pointed at the loopback port `withTestServer` bound.
// The surface is the small verb set a controller test drives — `get`/`post`/`patch`/`delete` — each
// returning a `TestResponse` that exposes the status, the raw body text, and typed JSON decoding.
